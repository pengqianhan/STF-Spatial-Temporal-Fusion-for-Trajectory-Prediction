...
def display_result(pra_results, pra_pref='Train_epoch'):
    ...
    overall_log_new = '|{}|[{}] RMSE second mean framerate new: {}'.format(datetime.now(), pra_pref, ' '.join(['{:.3f}'.format(x) for x in list(overall_loss_time_new) + [np.sum(overall_loss_time_new)]]))
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
    ...
    for iteration, (ori_data, A,A_big, _) in enumerate(pra_data_loader):
        ...
        for now_history_frames in range(history_frames,history_frames+1):
            ...
            predicted = pra_model(pra_x=input_data, pra_A=A,pra_A_big=A_big, pra_pred_length=output_loc_GT.shape[-2], pra_teacher_forcing_ratio=1, pra_teacher_location=output_loc_GT) # (N, C, T, V)=(N, 2, 6, 120)
    ...
    with open(test_result_file, 'w') as writer:
        ...
        for iteration, (ori_data, A,A_big, mean_xy) in enumerate(pra_data_loader):
            ...
            for t in range(future_frames):
                ...
                predicted = pra_model(pra_x=input_data, pra_A=A, pra_A_big=A_big, pra_pred_length=future_frames,pra_teacher_forcing_ratio=0,pra_teacher_location=future_trajectory)  # (N, C, T, V)=(N, 2, 6, 120)
...
if __name__ == '__main__':
    ...
    run_trainval(model, pra_traindata_path=traindata_path, pra_testdata_path='/mnt/DataSets/phan635_1/stfusion-dataset/test_data.pkl')
